...
function LiveStatImage({ src, alt, fallbackHref, fallbackLabel }) {
  const [failed, setFailed] = useState(false);

  if (failed) {
    return (
      <a
        className="activity-fallback"
        href={fallbackHref}
        target="_blank"
        rel="noreferrer"
      >
        <span className="tag">[unavailable]</span> {fallbackLabel} is
        temporarily down — view live on the source site →
      </a>
    );
  }
    ...
    <>
      ...
      <section className="hero">
        ...
        <div className="hero-inner">
          ...
          <p className="hero-tagline">
            <span className="tagline-prefix">&gt;</span> {profile.tagline}
          </p>

          <div className="featured-skills">
            {featuredSkills.map((s) => (
              <span className="featured-chip" key={s}>
                {s}
              </span>
            ))}
          </div>
      ...
      <section id="projects" className="section">
        <div className="wrap-wide">
          ...
          <div className="notice">
            ...
            two client-adjacent builds below; a third personal project slot
            is still being curated.
          </div>
          <div className="project-grid">
            {projects.map((p) =>
              p.title ? (
                <div className="project-card" key={p.slot}>
      ...
      <section id="activity" className="section">
        <div className="wrap-wide">
          <div className="cmd-line">
            <span className="prompt">$</span>
            <span className="cmd">./stats --github=static --leetcode=live</span>
          </div>
          <div className="activity-grid">
            <img
              className="activity-card"
              src={profile.githubStatsImg}
              alt="GitHub stats for SigmaRajput — 46 public repos, 6 followers, 5 following"
              loading="lazy"
            />
            <img
              className="activity-card"
              src={profile.githubLangsImg}
              alt="Repository languages for SigmaRajput"
              loading="lazy"
            />
            <LiveStatImage
              src={`https://leetcard.jacoblin.cool/${profile.leetcodeUsername}?theme=dark&font=JetBrains%20Mono&extension=activity`}
              alt="LeetCode stats for Sigma-lord"
              fallbackHref={`https://leetcode.com/u/${profile.leetcodeUsername}/`}
              fallbackLabel="LeetCode stats card"
            />
          </div>

          <div className="cmd-line" style={{ marginTop: 40 }}>
            <span className="prompt">$</span>
            <span className="cmd">less resume.pdf</span>
          </div>
          <div className="resume-frame">
            <iframe
              src={profile.resumePreviewUrl}
              title="Ravi Singh — Resume preview"
              loading="lazy"
            />
          </div>
        </div>
